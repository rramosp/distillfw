...
echo "--------------------------------------------------------------------------------"
if [ -n "$BACKEND_URL" ]; then
echo "  ► Google Cloud Run Hosted Endpoints (IAM Protected):"
echo "      - Backend API:  ${BACKEND_URL}/api"
echo "      - Frontend UI:  ${FRONTEND_URL}"
echo ""
echo "      [IMPORTANT: Accessing Cloud Run in Enterprise GCP Organizations]"
echo "      Direct browser requests to *.run.app will return 'Error: Forbidden' because"
echo "      organization policy 'constraints/iam.allowedPolicyMemberDomains' prohibits"
echo "      unauthenticated public access (allUsers)."
echo ""
echo "      To access the Cloud Run service in your browser with automatic authentication:"
echo "        gcloud run services proxy distillfw-backend --region=us-central1 --port=8080"
echo "        -> Then open: http://localhost:8080"
echo ""
echo "      To query the Cloud Run API via cURL with identity token:"
