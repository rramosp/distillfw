...
class TeacherInferenceService:
    ...
    def _call_gemini_api(
        ...
        """Attempts calling Vertex AI / Google GenAI SDK, or falls back to simulation."""
        full_prompt = f"{instructions}\n\nProblem:\n{prompt}\n\nSolution:"
        
        # Try using google-genai or vertexai if configured
        api_key = os.getenv("GEMINI_API_KEY")
        if api_key or settings.GCP_PROJECT_ID:
            try:
                from google import genai
                from google.genai import types

                client = genai.Client(
                    vertexai=bool(settings.GCP_PROJECT_ID and not api_key),
    ...
    def run_inference_job(
        ...
        try:
            ...
            total = len(rows)
            operations_logger.log(f"Processing {total} prompts with Teacher Model '{model_name}'", level="INFO", source="TEACHER", project_id=project_id)
            ...
            operations_logger.log(
                f"Teacher inference completed for {total} prompts. Total tokens: {total_prompt_tokens + total_completion_tokens}",
                level="SUCCESS",
                source="TEACHER",
                project_id=project_id
            )
            ...
            storage_service.record_history(
                ...
                f"Generated teacher inferences with reasoning traces in data/teacher_inferences.jsonl",
                start_time
            )
